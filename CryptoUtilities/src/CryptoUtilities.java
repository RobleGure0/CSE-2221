...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        boolean resultIsEven = false;
        NaturalNumber found = new NaturalNumber2(n);
        NaturalNumber two = new NaturalNumber2(2);
        if (found.divide(two).isZero()) {
            resultIsEven = true;
        }
        return resultIsEven;
    }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        newN.copyFrom(n);
        newW.copyFrom(w);
        powerMod(newW, two, newN);

        if (newW.equals(one)) {
            isComposite = true;
        } else {
            newN.decrement();
            powerMod(newW, newN, n);
            if (!newW.equals(one)) {
                isComposite = true;
            }
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        for (int i = 0; i < maxIterations; i++) {
            ...
            while (w.compareTo(upperBound) >= 0) {
                w.decrement();
            }
